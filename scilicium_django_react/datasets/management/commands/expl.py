...
def import_onto(dict_onto,user):
    ...
    if dict_onto["model"] != "skip" :
        ...
        try : 
            ...
            if not className.objects.filter(displayLabel = dict_onto['displayLabel']).exists() and not className.objects.filter(displayLabel = dict_onto["displayLabel"].lower()).exists() : 
            ...
            #si le display label lower case existe ,  s'il y a un ONTO ID de transmis et s'il n'y en as pas sur l'existant, on l'ajoute
            elif className.objects.filter(displayLabel = dict_onto["displayLabel"].lower()).exists() and dict_onto["ontologyID"] != "" and className.objects.filter(ontologyID = "") and dict_onto["displayLabel"] != "" :
                ...
                updateOntoID(sp,dict_onto["ontologyID"],dict_onto["ontologyLabel"])
            ...
            elif className.objects.filter(displayLabel = dict_onto["displayLabel"]).exists() and dict_onto["ontologyID"] != ""  and className.objects.filter(ontologyID = "") and dict_onto["displayLabel"] != "":    
